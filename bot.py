...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    as well as one-tap confirmations coming from the site (reg_<code> deep-links)."""
    ...
    # ── Пришёл с сайта: имя+послание уже собраны, нужно только подтвердить ──
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Processes sequential message steps one-by-one (fallback flow for people
    who type /start directly in the bot, without coming from the site)."""
